...
<div class="max-w-6xl mx-auto px-8 py-8">
    ...
    <div class="bg-[#F4F5F9] rounded-2xl p-8 shadow-sm">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-600 mt-1">Masukkan informasi organisasi, wilayah, kontak penanggung jawab, serta akun PIC balai.</p>
        </div>
        ...
        <form action="{{ route('balai.store') }}" method="POST">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="nama_balai" value="{{ old('nama_balai') }}" required placeholder="Contoh: BBWS Citarum"
                        ...
                        {{-- Balai no longer has its own login — accounts are created per-PIC below. --}}
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-white cursor-pointer hover:border-[#161446] transition">
                                    <input type="radio" name="unor" value="SDA" required {{ old('unor') == 'SDA' ? 'checked' : '' }}
                                ...
                                <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-white cursor-pointer hover:border-[#161446] transition">
                                    <input type="radio" name="unor" value="Binamarga" {{ old('unor') == 'Binamarga' ? 'checked' : '' }}
                                        class="w-4 h-4 text-[#161446] border-gray-300 focus:ring-[#161446]" />
                                    <span class="text-sm font-medium text-gray-800">Bina Marga</span>
                                </label>

                                <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-white cursor-pointer hover:border-[#161446] transition">
                                    <input type="radio" name="unor" value="Ciptakarya" {{ old('unor') == 'Ciptakarya' ? 'checked' : '' }}
                                        class="w-4 h-4 text-[#161446] border-gray-300 focus:ring-[#161446]" />
                                    <span class="text-sm font-medium text-gray-800">Cipta Karya</span>
                                </label>
                            </div>
                            @error('unor') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <select name="pulau" required
                            ...
                            @foreach($daftarPulau as $namaPulau)
                                <option value="{{ $namaPulau }}" @selected(old('pulau') == $namaPulau)>
                                    {{ $namaPulau }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="kepala" value="{{ old('kepala') }}" placeholder="Nama lengkap beserta gelar"
                                   class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:ring-2 focus:ring-[#161446] focus:outline-none" />
                            @error('kepala') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Kontak WhatsApp Kepala Balai</label>
                            <input type="text" name="kontak" value="{{ old('kontak') }}" placeholder="Contoh: 081234567890"
                                   class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:ring-2 focus:ring-[#161446] focus:outline-none" />
                            @error('kontak') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                ...
                {{-- Bagian Daftar PIC — each PIC gets its own login account (users table, role=pic) --}}
                <div>
                    <h2 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">Daftar Person In Charge (PIC)</h2>
                    <p class="text-xs text-gray-500 mb-4">Tambahkan minimal satu PIC agar Balai ini dapat login ke sistem.</p>

                    <div id="pic-container" class="space-y-4">
                        @php
                            $pics = old('pics', [['nama' => '', 'username' => '', 'password' => '', 'kontak' => '']]);
                        @endphp
                        ...
                        @foreach($pics as $index => $pic)
                            <div class="pic-row grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-4 items-end bg-gray-50 p-4 rounded-xl border border-gray-200 relative">
                                ...
                                <div>
                                    ...
                                    <input type="password" name="pics[{{ $index }}][password]" required placeholder="********"
                                           class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:ring-2 focus:ring-[#161446] focus:outline-none" />
                                </div>
                                <div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addButton.addEventListener('click', function() {
        ...
        row.innerHTML = `
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Password</label>
                <input type="password" name="pics[${picIndex}][password]" required placeholder="********"
                       class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:ring-2 focus:ring-[#161446] focus:outline-none" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Kontak WhatsApp</label>
